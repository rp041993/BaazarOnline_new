...
<!--                                    <li  id="hello"  onclick="sendInfo(this.id);"  class="add"></li> 
                                ...
                                <input type="text" id="001" name="attr" value="<%=att%>">
...
function sendInfo(id,value) {
     ...
     $.ajax({
        ...
        url: 'ChosseCatagory?cid='+id+'&attibuteset='+value,
        type: 'GET',
         dataType: 'json', 
         async: false, success: function(result){;
             ...
             if (result.jsonArray.length===24) {
              ...
              $("#searchListUl").empty();
              if (type==="product") {
                  var url = 'addproductAdmin.jsp?AttributeSet='+att+'&cid='+id
              $("#searchListUl").append('<li "><a href="'+url+'">add</a></li>');
   
          }  else if(type==="comision") {
               var url = 'ShippingCalculator.jsp?name='+att+'&val='+id
              $("#searchListUl").append('<li "><a href="'+url+'">add commions</a></li>');
            
                }
                else if(type==="export") {
               var url = '${pageContext.request.contextPath}/ProductExportFormate?attributeSet='+att+'&val='+id+'&name=amit'
              $("#searchListUl").append('<li "><a href="'+url+'">export</a></li>');
